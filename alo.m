...
for it=1:MaxIt
    ...
    for i=1:nPop,
      ...
      % RA is the random walk around the selected antlion by rolette wheel
        RA=Random_walk_around_antlion(model.m,MaxIt,VarMin,VarMax, antlion(Rolette_index).Position,it);
        
        % RA is the random walk around the elite (best antlion so far)
        RE=Random_walk_around_antlion(model.m,MaxIt,VarMin,VarMax, BestSol.Position,it);
        ...
        % Evaluation
        [ant(i).Cost, ant(i).Sol] = CostFunction(ant(i).Position);
    ...
    % Show Iteration Information
     if (rem(it,refresh) == 0)
        ...
        str = sprintf('QAP with ALO : %.3f',BestSol.Cost');
%         title(str);
        text(-5,15,str);
        pause(0.01);
     end
