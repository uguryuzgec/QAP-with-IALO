...
for it=1:MaxIt
    ...
    for i=1:nPop,
        ...
        RA=Random_walk_NEW(model.m,MaxIt,VarMin,VarMax,antlion(Select_Index(i)).Position,it,X);
        
        % RA is the random walk around the elite (best antlion so far)
        RE=Random_walk_NEW(model.m,MaxIt,VarMin,VarMax,BestSol.Position,it,Y);
    ...
    % Show Iteration Information
     if (rem(it,refresh) == 0)
        ...
        str = sprintf('QAP with TALO : %.3f',BestSol.Cost');
%         title(str);
        text(-5,15,str);
        pause(0.01);
     end
